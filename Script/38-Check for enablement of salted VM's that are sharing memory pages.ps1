...
$fileName = "38-Check for enablement of salted VM's that are sharing memory pages.txt"
...
$commandOutput1 = write-Output "38-Check for enablement of salted VM's that are sharing memory pages"
$commandOutput2 = Get-VM | Get-AdvancedSetting -Name "Mem.ShareForceSalting" | where {$_.Value -eq "1"} | Select Entity, Name, Value
