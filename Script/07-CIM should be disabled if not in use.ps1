...
$fileName = "07-CIM should be disabled if not in use.txt"
...
$commandOutput1 = write-Output "07-CIM should be disabled if not in use"
$commandOutput2 = Get-VMHostService -VMHost * | Where-Object {$_.Key -eq 'sfcbd-watchdog' -and $_.Running -eq 'True'}
$commandOutput3 = Get-VMHostService -VMHost * | Where-Object {$_.Key -eq 'sfcbd-watchdog' -and $_.Policy -eq 'On'}
