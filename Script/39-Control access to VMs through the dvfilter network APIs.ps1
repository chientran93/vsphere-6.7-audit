...
$fileName = "39-Control access to VMs through the dvfilter network APIs.txt"
...
$commandOutput1 = write-Output "39-Control access to VMs through the dvfilter network APIs"
$commandOutput2 = Get-VM | Get-AdvancedSetting -Name  "ethernet*.filter*.name*" | Select Entity, Name, Value
